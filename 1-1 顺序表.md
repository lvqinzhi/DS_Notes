...
初始化时将last赋值为-1，即最后一个元素在-1的位置，即表中没有元素；插入元素时，先判断位置是否合法、表是否满，如符合要求，则将插入位置及后面的元素都向后移动一位，再将元素放在该位置，并且last指针加一；删除则相反，只要位置合法，直接把删除位置后面的元素向前移动一位即可，需要的删除的元素就会被覆盖掉，last指针减一。
